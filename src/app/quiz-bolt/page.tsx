...
import { Puzzle, Users, IndianRupee, Clock, Loader2 } from "lucide-react";
...
function QuizBoltPageContent() {
  ...
  const handleRegister = (tournamentId: string, fee: number) => {
    toast({
      title: "Registration Successful!",
      description: `You have been registered for the tournament. Your wallet has been debited by ₹${fee}.`,
    });
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-4xl font-bold text-primary flex items-center justify-center gap-3">
          <Puzzle className="w-10 h-10" /> Quiz Bolt
        </h1>
        ...
        <div className="space-y-4">
          {tournaments.map((tourney) => (
            <Card key={tourney.id} className="shadow-md">
              ...
              <CardFooter>
                <Button className="w-full" onClick={() => handleRegister(tourney.id, tourney.entryFee)}>
...
export default function QuizBoltPage() {
    return (
        <ProtectedRoute>
            <QuizBoltPageContent />
        </ProtectedRoute>
