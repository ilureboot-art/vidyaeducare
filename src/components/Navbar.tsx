...
const navItems = [
  ...
  { href: "/storyteller", label: "StoryTeller", icon: Mic2 },
  { href: "/consultancy", label: "Consultancy", icon: HeartHandshake },
...
export function Navbar() {
  ...
  if (
    ...
    pathname.startsWith("/quiz-clash/play") ||
    pathname.startsWith("/trial-mock-test")
  ) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-2 z-50 shadow-[0_-5px_15px_-3px_rgba(0,0,0,0.05)]">
      <nav className="container mx-auto grid grid-cols-4 sm:grid-cols-8 gap-1 max-w-3xl">
        {navItems.map((item) => {
