...
import { PlusCircle, Trash2, Zap, BookOpen, GraduationCap, Percent, Loader2, IndianRupee, RefreshCcw, Landmark, ShieldCheck, FileText, Share2, Clock } from "lucide-react";
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card className="mt-6">
          <CardHeader>
              ...
              <CardDescription>Adjust commission rates for Paid and Free IBAs.</CardDescription>
          </CardHeader>
          <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 border rounded-xl bg-primary/[0.02]">
                  <div className="space-y-2">
                      <Label htmlFor="iba-comm-rate" className="font-bold flex items-center gap-2">
                          <Percent size={14} className="text-primary"/> Paid IBA Commission (%)
                      </Label>
                      <Input 
                        id="iba-comm-rate" 
                        type="number" 
                      ...
                      />
                      <p className="text-[10px] text-muted-foreground italic">Applied to sales generated by associates who purchased a mock test package.</p>
                  </div>
                  <div className="space-y-2">
                      <Label htmlFor="free-iba-comm-rate" className="font-bold flex items-center gap-2">
                          <Percent size={14} className="text-primary"/> Free/Unpaid IBA Commission (%)
                      </Label>
                      <Input 
                        id="free-iba-comm-rate" 
                        type="number" 
                        value={storeConfig.freeIbaCommissionRate ?? 5} 
                        onChange={(e) => setStoreConfig(prev => prev ? ({...prev, freeIbaCommissionRate: Number(e.target.value)}) : null)} 
                      />
                      <p className="text-[10px] text-muted-foreground italic">Applied to sales generated by associates who have not yet purchased a package.</p>
                  </div>
              </div>
          </CardContent>
        </Card>
        ...
        <Card className="mt-6">
          <CardHeader>
              <CardTitle className="flex items-center gap-2"><BookOpen size={20} className="text-primary"/> Mock Test Free Trial Settings</CardTitle>
              <CardDescription>Adjust the number of days of free access given to newly created student workspaces.</CardDescription>
          </CardHeader>
          <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 border rounded-xl bg-primary/[0.02]">
                  <div className="space-y-2">
                      <Label htmlFor="free-trial-days" className="font-bold flex items-center gap-2">
                          <Clock size={14} className="text-primary"/> Free Trial Period (Days)
                      </Label>
                      <Input 
                        id="free-trial-days" 
                        type="number" 
                        value={storeConfig.freeTrialDays ?? 30} 
                        onChange={(e) => setStoreConfig(prev => prev ? ({...prev, freeTrialDays: Number(e.target.value) || 0}) : null)} 
                      />
                      <p className="text-[10px] text-muted-foreground italic">Students get this many days of free access to mock tests starting from their workspace creation date.</p>
                  </div>
              </div>
          </CardContent>
        </Card>
