...
export default function StorytellerPage() {
  ...
  const selectedVoiceSample = config.voiceSamples.find(sample => sample.language === form.language);
  ...
  return <main className="mx-auto max-w-6xl p-4 md:p-8 space-y-8">
    <section className="text-center space-y-3"><Badge>ALL USER REELS ARE PAID</Badge><h1 className="text-4xl md:text-6xl font-black text-primary"><Mic2 className="inline mr-3" />{config.productName}</h1><p className="text-xl text-muted-foreground">{config.tagline}</p><p className="font-black text-2xl">₹{config.reelPrice} / Reel</p>{config.enabled && <div className="flex justify-center"><PromotionShare title={`Vidya Educare – ${config.productName}`} description="Create your own story-to-audio reel. Login and payment are required before generation. View the current price and demo online." path="/storyteller" imagePath="/promotions/storyteller-ai.webp" /></div>}</section>
    {config.demoEnabled && config.demoAssetPath && <Card id="storyteller-demo"><CardHeader><CardTitle>{config.demoTitle}</CardTitle><CardDescription>{config.demoDescription}</CardDescription></CardHeader><CardContent className="space-y-4"><video className="w-full max-h-[70vh] rounded-xl bg-black" controls poster={config.demoThumbnail || undefined} src={config.demoAssetPath} /><PromotionShare title="Vidya Educare StoryTeller AI demo" description="Listen to the free demo to experience story-aware narration and sound design. Creating your own reel requires login and payment; check the current price online." path="/storyteller#storyteller-demo" imagePath="/promotions/storyteller-ai.webp" /></CardContent></Card>}
    {!user ? <Card className="text-center"><CardHeader><CardTitle>Login to create your Reel</CardTitle><CardDescription>Watch the demo for free. Your own Reel requires wallet payment before generation.</CardDescription></CardHeader><CardContent><Button asChild size="lg"><Link href="/login?returnTo=/storyteller">Create Your Reel – ₹{config.reelPrice}</Link></Button></CardContent></Card> : <>
      <Card><CardHeader><CardTitle>Create Your Story to Audio Reel</CardTitle><CardDescription>Configure first. No paid AI API is called until wallet payment succeeds.</CardDescription></CardHeader><CardContent className="space-y-5">
        <div className="grid md:grid-cols-2 gap-4"><Field label="Story title"><Input value={form.title} maxLength={120} onChange={e => setForm({...form,title:e.target.value})}/></Field><Field label="Language"><Picker value={form.language} values={config.languages} onChange={v=>setForm({...form,language:v,voice:config.voices.find(voice=>voice.language===v)?.id||""})}/></Field></div>
        {selectedVoiceSample && <div className="rounded-xl border bg-muted/40 p-4"><p className="font-bold">{selectedVoiceSample.label}</p><p className="text-xs text-muted-foreground mb-2">Narration sample for {selectedVoiceSample.language}. The reel uses the configured audio narration service.</p><audio controls preload="none" className="w-full" src={selectedVoiceSample.assetPath}/></div>}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4"><Field label="Narrator"><Picker value={form.voice || "default"} values={config.voices.filter(v=>v.language===form.language).map(v=>v.id).length ? config.voices.filter(v=>v.language===form.language).map(v=>v.id) : ["default"]} onChange={v=>setForm({...form,voice:v==="default"?"":v})}/></Field><Field label="Narration tone"><Picker value={form.voiceStyle} values={config.voiceStyles} onChange={v=>setForm({...form,voiceStyle:v})}/></Field><Field label="Duration"><Picker value={String(form.duration)} values={[...config.durations.map(String),...(config.allowAutoDuration?["AUTO"]:[])]} onChange={v=>setForm({...form,duration:v==="AUTO"?"AUTO":Number(v)})}/></Field><Field label="Music"><Picker value={form.music} values={config.musicCategories} onChange={v=>setForm({...form,music:v})}/></Field><Field label="Template"><Picker value={form.template} values={config.templates} onChange={v=>setForm({...form,template:v})}/></Field></div>
