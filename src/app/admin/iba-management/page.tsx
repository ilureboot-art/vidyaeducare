...
export default function IbaManagementPage() {
  ...
  const calculatePayout = async (iba: any) => {
    ...
    const fixedRemuneration = iba.status === "APPROVED" && window.confirm("Include the approved Fixed Monthly Remuneration in this payout?") ? policy.fixedMonthlyRemuneration : 0;
  ...
  return <div className="space-y-6 max-w-7xl mx-auto">
    ...
    <Tabs defaultValue="policy">
      ...
      <TabsContent value="ibas"><Card><CardContent className="pt-6 overflow-auto"><Table><TableHeader><TableRow><TableHead>IBA</TableHead><TableHead>Paid Active</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>{filteredIbas.map((iba) => <TableRow key={iba.uid}><TableCell><div className="font-medium">{iba.name}</div><div className="text-xs text-muted-foreground">{iba.email || iba.uid}</div></TableCell><TableCell>{iba.paidActive ? "Yes" : "No"}</TableCell><TableCell><Badge variant="outline">{iba.status || "TRAINING"}</Badge></TableCell><TableCell className="flex flex-wrap gap-1"><Button size="sm" onClick={() => eligibilityAction(iba.uid,"APPROVED")}>Approve</Button><Button size="sm" variant="outline" onClick={() => eligibilityAction(iba.uid,"NOT_ELIGIBLE")}>Reject</Button><Button size="sm" variant="destructive" onClick={() => eligibilityAction(iba.uid,"SUSPENDED")}>Suspend</Button><Button size="sm" variant="secondary" onClick={() => calculatePayout(iba)}>Calculate Payout</Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent>
